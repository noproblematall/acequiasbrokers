<div class="bg-white rounded-xl shadow-sm border border-gray-100 p-4">
            ...
            <div class="space-y-6">
            <!-- Background Color -->
            <div x-data="{ selectedColor: '{{ $section_4_bg_color }}' }">
                <label class="block text-sm font-medium text-gray-700 mb-3">Background Color</label>
                <div class="flex items-center space-x-4">
                    <div class="w-10 h-10 rounded-lg border-2 border-gray-300 shadow-sm overflow-hidden cursor-pointer hover:border-gray-400 transition-all duration-200" :style="'background-color: ' + selectedColor" wire:key="bg-color-{{ $section_4_bg_color }}">
                        <input type="color" wire:model="section_4_bg_color" x-model="selectedColor" class="opacity-0 w-full h-full cursor-pointer">
                    </div>
                    <div class="flex-1">
                        <div class="bg-gray-50 rounded-lg p-3 border border-gray-200 h-10 flex items-center">
                            <p class="text-sm font-medium text-gray-700">Selected Color: <span class="font-mono text-sm text-gray-600" x-text="selectedColor">{{ $section_4_bg_color }}</span></p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="border-t border-gray-200 pt-6">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-5 gap-4">
                    ...
                    <div>
                        ...
                        <div class="flex items-center space-x-3" x-data="{ selectedColor: '{{ $section_4_line_1_font_color }}' }">
                            <div class="w-10 h-10 rounded-lg border-2 border-gray-300 shadow-sm overflow-hidden cursor-pointer hover:border-gray-400 transition-all duration-200" :style="'background-color: ' + selectedColor" wire:key="line1-color-{{ $section_4_line_1_font_color }}">
                                <input type="color" wire:model="section_4_line_1_font_color" x-model="selectedColor" class="opacity-0 w-full h-full cursor-pointer">
                            </div>
                            <div class="flex-1">
                                <div class="bg-gray-50 rounded-md p-2 border border-gray-200 h-10 flex items-center">
                                    <p class="text-xs font-mono text-gray-600" x-text="selectedColor">{{ $section_4_line_1_font_color }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div class="border-t border-gray-200 pt-6">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-5 gap-4">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">Color</label>
                        <div class="flex items-center space-x-3" x-data="{ selectedColor: '{{ $section_4_line_2_font_color }}' }">
                            <div class="w-10 h-10 rounded-lg border-2 border-gray-300 shadow-sm overflow-hidden cursor-pointer hover:border-gray-400 transition-all duration-200" :style="'background-color: ' + selectedColor" wire:key="line2-color-{{ $section_4_line_2_font_color }}">
                                <input type="color" wire:model="section_4_line_2_font_color" x-model="selectedColor" class="opacity-0 w-full h-full cursor-pointer">
                            </div>
                            <div class="flex-1">
                                <div class="bg-gray-50 rounded-md p-2 border border-gray-200 h-10 flex items-center">
                                    <p class="text-xs font-mono text-gray-600" x-text="selectedColor">{{ $section_4_line_2_font_color }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div class="border-t border-gray-200 pt-6">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-6 gap-4">
                    <div>
                        ...
                        <div class="flex items-center space-x-3" x-data="{ selectedColor: '{{ $section_4_button_1_bg_color }}' }">
                            <div class="w-10 h-10 rounded-lg border-2 border-gray-300 shadow-sm overflow-hidden cursor-pointer hover:border-gray-400 transition-all duration-200" :style="'background-color: ' + selectedColor" wire:key="button-bg-{{ $section_4_button_1_bg_color }}">
                                <input type="color" wire:model="section_4_button_1_bg_color" x-model="selectedColor" class="opacity-0 w-full h-full cursor-pointer">
                            </div>
                            <div class="flex-1">
                                <div class="bg-gray-50 rounded-md p-2 border border-gray-200 h-10 flex items-center">
                                    <p class="text-xs font-mono text-gray-600" x-text="selectedColor">{{ $section_4_button_1_bg_color }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div>
                        ...
                        <div class="flex items-center space-x-3" x-data="{ selectedColor: '{{ $section_4_button_1_font_color }}' }">
                            <div class="w-10 h-10 rounded-lg border-2 border-gray-300 shadow-sm overflow-hidden cursor-pointer hover:border-gray-400 transition-all duration-200" :style="'background-color: ' + selectedColor" wire:key="button-text-{{ $section_4_button_1_font_color }}">
                                <input type="color" wire:model="section_4_button_1_font_color" x-model="selectedColor" class="opacity-0 w-full h-full cursor-pointer">
                            </div>
                            <div class="flex-1">
                                <div class="bg-gray-50 rounded-md p-2 border border-gray-200 h-10 flex items-center">
                                    <p class="text-xs font-mono text-gray-600" x-text="selectedColor">{{ $section_4_button_1_font_color }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="flex justify-end pt-4 border-t border-gray-200">
                    <button
                        type="button"
                        wire:click="updateSection4"
                        wire:loading.attr="disabled"
                        wire:target="updateSection4"
                        class="inline-flex items-center px-4 py-2 bg-orange-600 hover:bg-orange-700 disabled:opacity-50 disabled:cursor-not-allowed text-white font-medium text-sm rounded-lg transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-orange-500 cursor-pointer h-10">
                        <div wire:loading.remove wire:target="updateSection4">
                            <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path>
                            </svg>
                        </div>
                        <div wire:loading wire:target="updateSection4">
                            <svg class="animate-spin w-4 h-4 mr-2" fill="none" viewBox="0 0 24 24">
                                <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                                <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 818-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                            </svg>
                        </div>
                        <span wire:loading.remove wire:target="updateSection4">Update Section 4</span>
                        <span wire:loading wire:target="updateSection4">Updating...</span>
                    </button>
                </div>
            </div>
</div>